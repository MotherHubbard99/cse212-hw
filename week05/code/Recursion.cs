...
public static class Recursion
{
    ...
    public static void PermutationsChoose(List<string> results, string letters, int size, string word = "")
    {
        ...
        //recursive: try each letter that hasn't been used yet
        for (int i = 0; i < letters.Length; i++)
        {
            char chosen = letters[i];
            //remove the letter that was chosen
    ...
    public static decimal CountWaysToClimb(int s, Dictionary<int, decimal>? remember = null)
    {
        ...
        //memoization to check if already computed
    ...
    {
        //Base case: there are no wildcards left, so add it to the pattern
        if (!pattern.Contains('*'))
        {
            results.Add(pattern);
        }
